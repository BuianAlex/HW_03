...
class App extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.moodImg = ["tami-norm","51tami","tami-sick","tami-sleep","tami-died","tami-escape"];
  ...
  //приймаємо данні та фільтруємо данні з інпуту +заборона спец символів 
  ...
  //створбємо нового героя  
  createTami(e){
    e.preventDefault();
    this.setState({
      inputName: '',
      name:  this.state.inputName.length> 0 ? this.state.inputName.length: "Anonymous" ,
      health: 100,
      healthBtnState: '',
      fullness: 100,
      fullnessBtnState: '',
      ...
      return (
        <div>
          <div className="action-menu">
            ...
            <button className= {'circle-btn feed-btn'+this.state.fullnessBtnState} onClick={this.feedAction.bind(this)}>
              <span className="hint">Feed</span>
            </button>
            <button className= {"circle-btn cure-btn"+this.state.healthBtnState} onClick={this.cureAction.bind(this)}>
              <span className="hint">Cure</span>
            </button>
            <button className={"circle-btn fun-btn"+this.state.happinessBtnState} onClick={this.gameAction.bind(this)}>
              <span className="hint">Joy</span>
            </button>
            <button className={"circle-btn teach-btn"+this.state.wisdomBtnState} onClick={this.wisdomAction.bind(this)}>
              <span className="hint">Teach</span>
            </button>
            <button className={"circle-btn walk-btn"+this.state.workoutBtnState} onClick={this.workoutAction.bind(this)} >
              <span className="hint">Workout</span>
            </button>
            <button className={"circle-btn sleep-btn"+this.state.sleepBtnState} onClick={this.sleepAction.bind(this)}>
              <span className="hint">Sleep</span>
            </button>
            <button className={"circle-btn kill-btn"+this.state.killBtnState}  onClick={this.killTami.bind(this)}>
              <span className="hint">Kill</span>
            </button>
          </div>
          <ul className="tami-list">
            <li className = {`tami ${this.state.active}`} key='{this.props.key}' >
              <img src={`./img/${this.moodImg[this.state.mood]}.png`} className={this.state.wisdom === 100 ? "escape" : ""}></img>
